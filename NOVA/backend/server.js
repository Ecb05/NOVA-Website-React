...
/**
 * Team Registration endpoint with immediate response
 * Returns teamId instantly, processes Notion and email in background
 */
app.post('/api/register', (req, res) => {
  ...
  console.log('[REGISTER] Response sent! - Total time:', Date.now() - startTime, 'ms');
  
  // Process Notion and email in background (fire and forget)
  console.log('[REGISTER] Starting background processing');
  setImmediate(() => {
    processRegistrationAsync(teamId, teamName, teamLeaderEmail, members)
      .catch(error => {
        console.error('[REGISTER] Background error:', error);
      });
  });
});
